...
export function ShotCamera({ shot, onCapture, onClose }: { shot: Shot; onCapture: (photo: CapturedPhoto) => void; onClose: () => void }) {
  ...
  // sized to fill most of the live area between the top bar and the shutter.
  const topBar = 92;
  const bottomBar = 190;
  ...
  let guideW = liveW * (upright ? 0.72 : 0.94);
  let guideH = guideW / targetRatio;
  if (guideH > liveH * 0.92) {
    guideH = liveH * 0.92;
...
const s = StyleSheet.create({
  ...
  bottomBar: { position: "absolute", left: 0, right: 0, bottom: 0, height: 190, backgroundColor: "rgba(0,0,0,0.55)", alignItems: "center", paddingTop: 16, gap: 16 },
