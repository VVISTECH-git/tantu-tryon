 ...
 * One pack.
 *
 * Colour discipline: madder has exactly two jobs on this page — the
 * recommendation (badge, border, wash) and the value note under a unit price.
 * Navy is interactive, everything else is neutral. Anywhere madder appeared
 * without signalling one of those two things, it has been taken out.
 *
 * The recommendation is still marked four ways — worded badge, border, tinted
 * ground and a filled button — so it never rests on colour alone.
 */
  ...
  return (
    <article
      ...
      className={`group relative flex flex-col rounded-xl border transition duration-200 ease-out ${
        featured
          ? "border-madder/45 bg-madder-wash/70 shadow-[0_2px_10px_-4px_rgba(28,25,23,0.14)] hover:shadow-[0_10px_26px_-14px_rgba(28,25,23,0.26)] xl:-mt-3 xl:mb-3"
          : "border-line bg-surface hover:border-ink-faint/50 hover:shadow-[0_8px_22px_-16px_rgba(28,25,23,0.3)]"
      } motion-safe:hover:-translate-y-0.5`}
    >
      {featured && (
        <div className="flex justify-center">
          <span className="-mt-3 rounded-full bg-madder px-3.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-white">
            Most chosen
          </span>
        </div>
      )}
      ...
      <div className="flex flex-1 flex-col p-7">
        ...
        <p className="mt-2 min-h-5 text-[12.5px] font-medium leading-snug text-madder">{note}</p>
        ...
        {/* The honest note sits with the button, not three sections away. */}
        <p className="mt-3 text-center text-[12px] leading-snug text-ink-faint">
          We set it up with you — no instant checkout.
        </p>
        <p className="mt-1 text-center text-[12px] leading-snug text-ink-faint">{pack.microcopy}</p>
      </div>
    </article>
  );
}
