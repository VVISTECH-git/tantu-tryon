/*
  LOCKED, 26 Sep 2026. C1P2 (body photograph only, pose P2), prompt v7.1
  "rules first". The input and the rules it implies come first (body only:
...
export const C1P2_V71_VERSION = "v7.1-rules-first-LOCKED";

export function composeC1P2V71(x: {
  kind: string;
  fabricDesc: string;
  borderDesc: string;
  ...
  return [
    ...
    "ORIENTATION: On the skirt and in the pleats every motif stands exactly as in the photograph, pointing up toward the waist. Above the waist the fabric turns with the drape and the motifs turn with it. On the band of fabric across {her} chest, the upper edge is the hem border and the lower edge is the waist border, so the motifs point toward the band's lower edge. On the pallu hanging from {her} shoulder the fabric runs from top to bottom, so the motifs lie on their side: the long edge that continues up across the chest to the neck is the hem border, the outer long edge away from {her} body is the waist border, and every motif's tip points toward that outer edge, away from {her} body, all the way down the pallu.",
