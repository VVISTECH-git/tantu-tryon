...
describe("locked prompts", () => {
  it("C1P1 is exactly the locked v3 prompt", () => {
    const p1 = TEMPLATES.find((t) => t.id === "P1")!;
    const golden = readFileSync(join(__dirname, "__golden__", "C1P1-v3-locked.txt"), "utf8").replace(/\r/g, "");
    expect(composePrompt(p1, words, selections, [{ slot: "body", file: "b.png" }], plan)).toBe(golden);
    expect(promptVersion(p1, plan)).toBe("v3-one-print-LOCKED");
  });
});
